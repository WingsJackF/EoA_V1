...
def build_commands(python_executable: str, output_root: Path | None = None) -> list[list[str]]:
    ...
    for task in TASKS:
        ...
        if output_root is not None:
            command.extend(["--output-dir", str(output_root / task)])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run default experiments for CVRP/TSP LEHD/POMO tasks."
    )
    parser.add_argument(
        "--python",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Optional root directory for outputs; each run uses <root>/<task>.",
    )
