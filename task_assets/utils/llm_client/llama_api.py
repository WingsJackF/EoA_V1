...
class LlamaAPIClient(OpenAIClient):
    ...
    def __init__(
        ...
        if api_key is None:
            api_key = (
                get_env("LLAMA_API_KEY") if get_env is not None else os.getenv("LLAMA_API_KEY", None)
            )
            assert api_key, "Please provide llama API key via environment variable LLAMA_API_KEY"
        if base_url is None and get_env is not None:
            base_url = get_env("LLAMA_BASE_URL")
