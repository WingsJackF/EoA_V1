...
def _ensure_assets():
    ...
    missing_files = [
        str((p / "data" / f"test_TSP{problem_size}_n128.txt").relative_to(p))
        for problem_size in FULL_TEST_PROBLEM_SIZES
        if not (p / "data" / f"test_TSP{problem_size}_n128.txt").is_file()
    ]
    if missing_files:
        raise RuntimeError(
            "缺少 LEHD 测试数据文件："
            + ", ".join(missing_files)
            + "；请将数据放入 task_assets/problems/tsp_lehd/data"
        )
    return p
    ...
    with local_problem_environment(p), installed_module("gpt", candidate_module):
        ...
        for problem_size in problem_sizes:
            ...
            results[str(problem_size)] = {
                "teacher_score": float(score_optimal),
                "student_score": float(score_student),
                "gap_percent": float(gap),
                "combined_score": -float(score_student),
            }
    return results
...
def run_full_test(program_code: str, *, mode: str = "test") -> Dict[str, Any]:
    try:
        ...
        return {"mode": mode, "problem_sizes": {}, "mean_gap_percent": None, "error": str(e)}
