...
def _prepare_candidate(program_code: str):
    candidate_module = load_program_module(program_code, module_name="cvrp_pomo_candidate")
...
def run_evaluation(program_code: str) -> Dict[str, Any]:
    try:
        ...
        checkpoint = p / "checkpoints" / "checkpoint-30500.pt"
        ...
        with local_problem_environment(p), installed_module("gpt", candidate_module):
            for module_name in ("eval", "utils", "gen_inst", "CVRPTester", "CVRPModel", "CVRPEnv", "CVRProblemDef"):
            ...
            eval_mod.tester_params["test_data_load"]["filename"] = str(dataset_path)
